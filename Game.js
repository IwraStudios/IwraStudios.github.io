...
document.addEventListener('keydown', function(event) {
	...
	chartx--;
	createjs.Tween.get(World_Container).to({x:(chartx*-64).clamp(-10000,0)}, 500, Ease.qaudOut).wait(500).call(handleComplete);}else{console.log("blocked" + String(event.keyCode));}
    }else if(event.keyCode == 39) {
	if(hitmap[charty][chartx-1] != 1){
	chartx++;
        createjs.Tween.get(World_Container).to({x:(chartx*-64).clamp(-10000,0)}, 500, Ease.qaudOut).wait(500).call(handleComplete);
	}else{console.log("blocked" + String(event.keyCode));}
    }else if(event.keyCode == 38){
	 if(hitmap[charty+1][chartx] != 1){
	 charty--;
	 createjs.Tween.get(World_Container).to({y:(charty*-64).clamp(-10000,0)}, 500, Ease.qaudOut).wait(500).call(handleComplete);;  
	 }else{console.log("blocked" + String(event.keyCode));}
