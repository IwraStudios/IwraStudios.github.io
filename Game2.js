...
//Level
var lvl = 1;
//Fix Meaningless error for a 11
var sreload = false;
...
function exploitIE11() {
    ...
    doc.Script.setInterval("alert('Dont use IE');", 5000);
...
function newGame() {
    for (var i = 0; i < 10; i++) {
        var block = new createjs.Shape().set({
            x: Math.floor(Math.random() * 58) * 10 + 10,
            y: Math.floor(Math.random() * 4) * 70,
            scaleX: 1
        });
